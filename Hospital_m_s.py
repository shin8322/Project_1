...
class SendRow(QtWidgets.QWidget):
    ...
    def __init__(self, idx: int, parent=None):
        ...
        self.edit.setPlaceholderText("ASCII command/value (e.g., SM CH, 1500, done, AR, SET, AIRREP 3, AIRONLY)")
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Yeonjung Serial Tool - Servo Continuous + AR/SET/STM + AIRONLY")
        ...
        self._servo_send_timer.timeout.connect(self._servo_send_if_pending)  # ★ 존재해야 함
        ...
        cmd_box = QtWidgets.QGroupBox("Quick Commands (Firmware modes: SM/STM/AR/SET/AIRONLY)")
        ...
        self.btn_ar = QtWidgets.QPushButton("AR (AIRREP -> AR)")
        self.btn_aironly = QtWidgets.QPushButton("AIRONLY (AIRREP only)")  # ★ motor_mode=6
        ...
        cb.addWidget(self.btn_set, 0, 2)
        cb.addWidget(self.btn_ar, 0, 3)
        cb.addWidget(self.btn_aironly, 0, 4)
        ...
        cb.addWidget(self.btn_send_stm_cycle, 2, 2, 1, 3)

        cb.addWidget(QtWidgets.QLabel("Microstep"), 3, 0)
        cb.addWidget(self.cmb_step_mode, 3, 1)
        cb.addWidget(self.btn_send_step_mode, 3, 2, 1, 3)

        # AIRREP control
        self.spin_airrep = QtWidgets.QSpinBox()
        self.spin_airrep.setRange(0, 50)
        self.spin_airrep.setValue(int(self.state.airrep_count or 1))
        self.btn_airrep_apply = QtWidgets.QPushButton("Apply AIRREP")

        cb.addWidget(QtWidgets.QLabel("AIRREP (count)"), 4, 0)
        cb.addWidget(self.spin_airrep, 4, 1)
        cb.addWidget(self.btn_airrep_apply, 4, 2, 1, 3)

        # ★ 요청: AIRREP 0~9 버튼 추가 + 크기 키움
        pad = QtWidgets.QGroupBox("AIRREP Quick (0~9)")
        pad_layout = QtWidgets.QGridLayout(pad)
        pad_layout.setHorizontalSpacing(6)
        pad_layout.setVerticalSpacing(6)

        self.airrep_digit_buttons = []
        for d in range(10):
            b = QtWidgets.QPushButton(str(d))
            b.setMinimumSize(52, 44)   # ★ 크기 키움
            b.setStyleSheet("font-size: 16px; font-weight: 600;")  # ★ 글자 키움
            b.clicked.connect(lambda _=False, dd=d: self.set_and_send_airrep_digit(dd))
            self.airrep_digit_buttons.append(b)
            pad_layout.addWidget(b, d // 5, d % 5)

        cb.addWidget(pad, 5, 0, 1, 5)
        ...
        self.spin_repeat.setRange(0, 100000)
        self.spin_repeat.setValue(0)
        ...
        self.btn_set.clicked.connect(lambda: self.enqueue_send_ascii("SET"))
        ...
        self.btn_ar.clicked.connect(self.send_ar_with_airrep)

        # ★ AIRONLY
        self.btn_aironly.clicked.connect(self.send_aironly_with_airrep)
    ...
    def restore_rows(self):
        ...
        seed = [
            "SET", "AIRREP 3", "AIRONLY", "AR", "SM CH",
            "1500", "1600", "done", "STM SET"
        ]
    ...
    # AIRREP digit pad
    # -----------------------------
    def set_and_send_airrep_digit(self, d: int):
    ...
    def send_aironly_with_airrep(self):
        rep = int(self.spin_airrep.value())
        self.enqueue_send_ascii(f"AIRREP {rep}")
        self.enqueue_send_ascii("AIRONLY")
    ...
    def run_checked(self):
        ...
        if repeat == 0:
            ...
            self.log(f"[{ts()}] INFO: Repeat=0 -> nothing to run.")
            return
    ...
    def _schedule_servo_send(self):
        ...
        if not self._servo_send_timer.isActive():
            self._servo_send_timer.start(40)  # 25Hz max

    def _servo_send_if_pending(self):
        # ★ 에러났던 부분: 반드시 메서드가 존재해야 함
